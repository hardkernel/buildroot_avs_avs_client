...
std::unique_ptr<SampleApplication> SampleApplication::create(
    ...
    if (!clientApplication->initialize(configFiles, pathToInputFolder, logLevel, runMode)) {
...
bool SampleApplication::initialize(
    ...
    std::shared_ptr<alexaClientSDK::sampleApp::PortAudioMicrophoneWrapper> micWrapper = 
            alexaClientSDK::sampleApp::PortAudioMicrophoneWrapper::create(sharedDataStream, &notify_detector, (std::shared_ptr<alexaClientSDK::sampleApp::UIManager>)userInterfaceManager);
    if (!micWrapper) {
...
// Creating wake word audio provider, if necessary
    ...
    kwd::DSPKeyWordDetector::DSPConfiguration dsp_config;
    ...
    m_userInputKeyManager = alexaClientSDK::sampleApp::UserInputKeyManager::create(m_interactionManager);
